...
ITStatus USART_GetITStatus(USART_TypeDef* USARTx, uint16_t USART_IT)
{
  ...
  /* Get the USART register index */
  usartreg = (((uint8_t)USART_IT) >> 0x05); //D7~D5:代表中断标志位对应的中断使能位在CR1、CR2还是CR3寄存器中
  /* Get the interrupt position */
  itmask = USART_IT & IT_Mask;
  itmask = (uint32_t)0x01 << itmask; //D4~D0:代表中断标志位对应的中断使能位在CRx寄存器的哪一位
  //位5:当USART_SR中的ORE或者RXNE为’1’时，产生USART中断。 
  if (usartreg == 0x01) /* The IT  is in CR1 register */
  {
    itmask &= USARTx->CR1;
  }
  ...
  bitpos = USART_IT >> 0x08; //D15~D8:代表中断标志位在SR寄存器中的哪一位
  bitpos = (uint32_t)0x01 << bitpos;
  bitpos &= USARTx->SR; //位5:收到数据，可以读出
