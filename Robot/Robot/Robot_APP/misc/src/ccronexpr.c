...
#define cron_malloc(x) sys_malloc(x);
#define cron_free(x) sys_free(x);
...
static time_t timegm(struct tm* __tp)
{
    const int32_t days[2][12] = {
                    { 0,31,59,90,120,151,181,212,243,273,304,334},
                    { 0,31,60,91,121,152,182,213,244,274,305,335}
                };
    int year = __tp->tm_year + YEAR0;
    int month = __tp->tm_mon;
    time_t result;
    ...
    result = days_from_0(year) - days_from_0(EPOCH_YEAR); // year to days
    result += days[LEAPYEAR(year)][month] + __tp->tm_mday - 1; // day of year
    result *= SECS_DAY; // day to seconds
    result += 3600 * __tp->tm_hour + 60 * __tp->tm_min + __tp->tm_sec;
    return result;
}
...
static struct tm *gmtime_r(const time_t *timep, struct tm *result)
{
    time_t secs = *timep;
    int year = EPOCH_YEAR;
    unsigned long dayclock = (unsigned long)secs % SECS_DAY;
    unsigned long dayno    = (unsigned long)secs / SECS_DAY;
    const int _ytab[2][12] = {
            {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31},
            {31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}
        };

    result->tm_sec  = (int) dayclock % 60;
    result->tm_min  = (int)(dayclock % 3600)/60;
    result->tm_hour = (int) dayclock / 3600;
    result->tm_wday = (int) (dayno + 4) % 7;        /* day 0 a Thursday */
    ...
    result->tm_yday = (int)dayno;
    result->tm_mon  = 0;

    while (dayno >= (unsigned long)_ytab[LEAPYEAR(year)][result->tm_mon]) {
        dayno -= _ytab[LEAPYEAR(year)][result->tm_mon];
        result->tm_mon ++;
    }

    result->tm_mday  = (int)(dayno + 1);
    result->tm_isdst = 0;
    return result;
}
